		...
		if (effects.getTamed().isPresent()) {
			...
			if (!effects.entity.worldObj.isRemote) {
				...
				if (effects.entity.getRNG().nextInt(3) == 0) {
					EntityTame tameStat = new EntityTame(EntityPlayer.func_146094_a(player.getGameProfile()),
							getDefaultEntityName(), Command.FOLLOWING);
					effects.setTamed(tameStat);
...
//					PZPacket packet = new PacketSyncTame().setPacketData(effects.entity.getEntityId(), tameStat);
