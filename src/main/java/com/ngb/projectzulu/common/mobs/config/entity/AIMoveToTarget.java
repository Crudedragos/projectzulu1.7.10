...
public class AIMoveToTarget extends AIBase {
	...
	public AIMoveToTarget(EntityEffects entityEffects, IEntitySelector targetFilter) {
		this(entityEffects, targetFilter, 1.0f, false);
	}

	public AIMoveToTarget(EntityEffects entityEffects, IEntitySelector targetFilter, float speedModifier,
	...
	public boolean shouldExecute() {
		...
		if (target.isPresent() && targetFilter.isEntityApplicable(target.get())) {
			return effects.isGrounded() ? effects.entity.getNavigator().getPathToEntityLiving(target.get()) != null
					: true;
		} else {
			return false;
		}
